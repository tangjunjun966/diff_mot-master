...
def mot_vision_demo():
    ...
    '''
    root=r'E:\project\project_tracker\data\MOT17\train\MOT17-02-FRCNN'


    out_dir_gt=build_dir(os.path.join(root,'out_dir_draw','draw_gt_imgs'))
    out_dir_det=build_dir(os.path.join(root, 'out_dir_draw', 'draw_det_imgs'))

    path_det_txt=os.path.join(root,'det\det.txt')
    path_gt_txt = os.path.join(root, 'gt\gt.txt')
    path_img=os.path.join(root,'img1')

    det_info=read_txt(path_det_txt)
    gt_info=read_txt(path_gt_txt)
    det_val_lst=np.array([[float(v) for v in det.split(',')] for det in det_info ])
    gt_val_lst = np.array([[float(v) for v in gt.split(',')] for gt in gt_info])
