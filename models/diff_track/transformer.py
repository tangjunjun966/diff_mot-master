...
def multi_head_attention_forward(
    ...
    attn_output, attn_output_weights = scaled_dot_product_attention(q, k, v, attn_mask, dropout_p)    #q,k,v 计算V = qkV
    attn_output = attn_output.transpose(0, 1).contiguous().view(tgt_len, bsz, embed_dim)
    attn_output = F.linear(attn_output, out_proj_weight, out_proj_bias)    #计算出来的值增加一个linear
...
class MultiheadAttention(Module):
    ...
    Outputs:
        ...
        if not self._qkv_same_embed_dim:
            attn_output, attn_output_weights = multi_head_attention_forward(          # F.multi_head_attention_forward(
